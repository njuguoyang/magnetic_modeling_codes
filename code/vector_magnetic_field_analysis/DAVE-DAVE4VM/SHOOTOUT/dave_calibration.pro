pro dave_calibration
  ...
  helicityscl=1.d37
  DIR='SCHUCK/'
  dave=mrdfits(DIR+'dave_optimize.fts',1)
  opt=mrdfits(DIR+'dave.fts',3)
  set_plot,'x'
  plot,dave.window_size[0],dave.helicity_d/helicityscl
  YC=!Y.CRANGE
  !P.font=1
  th=4
  ...
  set_plot,'ps'
  loadct,39
  device,filename=DIR+'f1c.eps',/encapsulate,/color,bits_per_pixel=8
  !X.MARGIN=[6,5]
  yr=[min([0,dave.poynting_d,dave.poynting_anmhd],MAX=MX),MX]/ergscl
  plot,dave.window_size[0],dave.poynting_d/ergscl,$
...
;
  NN=N_ELEMENTS(dave)
  oplot,dave.window_size[0],replicate(dave[0].poynting_anmhd/ergscl,NN),$
...
;
  ...
  hc=254
  yr=[min([0,dave.helicity_d,dave.helicity_anmhd],MAX=MX),MX]/helicityscl
  axis,XR1[1],0,/yaxis,/data,_extra=extra,/save,color=hc,$
       ytitle=method+'Helicity Flux [10!U37!N Wb!U2!N/s]',yrange=yr
  oplot,dave.window_size[0],dave.helicity_d/helicityscl,_extra=extra,$
        color=hc
  oplot,dave.window_size[0],replicate(dave[0].helicity_anmhd/helicityscl,NN),$
            _extra=extra,linestyle=2,color=hc
  device,/close_file

  plot,dave.window_size[0],dave.induction_L[1]
  YC=!Y.CRANGE
  device,filename=DIR+'f1a.eps',/encapsulate
  !X.MARGIN=[7,7]
  plot,dave.window_size[0],dave.induction_c,$
        xtitle='Window Size [Pixels]',xstyle=1,$
        ytitle=method+'Correlation',yrange=[-1,0],$
       _extra=extra,color=0,ystyle=9,position=aspect(1.d0)
  oplot,dave.window_size[0],dave.induction_r,_extra=extra,linestyle=2
  XR1=!X.crange
  YR1=!Y.crange
  oplot,opt.window_size,YR1,_extra=extra,linestyle=3
  hc=254
  yr=[min([dave.induction_L[1],-.92,-1.02],MAX=MX),MX]
  axis,XR1[1],0,/yaxis,/data,yrange=YR,_extra=extra,ystyle=1,/save,color=hc,$
       ytitle=method+'Slope'
  oplot,dave.window_size[0],dave.induction_L[1],_extra=extra,$
        color=hc
